...
it('serve + server.file writes a proxy to outDir and does not serve HMR', async () => {
  ...
  try {
    ...
    expect(installBody).toContain('file://')
    expect(installBody).toContain('vanilla.js')
    expect(hmr.status).toBe(404)
    expect(virtual?.code).toContain('"file":"vanilla.proxy.user.js"')
